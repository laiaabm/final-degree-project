...
from aggregator import GMA, TransMIL, DSMIL, CLAM_SB, CLAM_MB, VarAttention, GTP, PatchGCN_Surv, DeepGraphConv_Surv, MIL_Sum_FC_surv, MIL_Attention_FC_surv, MIL_Cluster_FC_surv, PureTransformer, ViT, Transformer#, Transformer2
...
def get_aggregator(method='', ndim=1024, n_classes=2, **kwargs):
    ...
    if method == 'AB-MIL':
        ...
        return MIL_Attention_FC_surv(ndim=ndim, n_classes=n_classes, size_arg='small', **kwargs)
    elif method == 'AB-MIL_FC_big':
        return MIL_Attention_FC_surv(ndim=ndim, n_classes=n_classes, size_arg='big', **kwargs)
    # DeepSMILE
    elif method == 'VarMIL':
    ...
    if method == 'CLAM_SB':
        ...
        return PureTransformer(ndim=ndim, n_classes=n_classes, **kwargs)
    # TransMIL
    elif method == 'transMIL':
        return TransMIL(ndim=ndim, n_classes=n_classes, **kwargs)
    # VIT 
    elif method == 'ViT':
        return ViT(ndim=ndim, n_classes=n_classes, **kwargs)
    # Transformer
    elif method == 'Transformer':
        return Transformer(ndim=ndim, n_classes=n_classes, **kwargs)
    # Transformer2
    # elif method == 'Transformer2':
    #     return Transformer2(ndim=ndim, n_classes=n_classes, **kwargs)
    # DSMIL
    elif method == 'DS-MIL':
